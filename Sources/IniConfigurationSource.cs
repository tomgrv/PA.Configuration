...
namespace PA.Configuration
{
	public partial class IniConfigurationSource : Component, IConfigurationSource
	{
		...
		{
			this.FileName = Process.GetCurrentProcess().ProcessName + ".ini";
		}
		...
		public void Open()
		{
			...
			{
				Trace.TraceInformation("Loading configuration from " + this.FileName);
				this.settings = new Settings(this.FileName, Settings.Format.IniFormat);
